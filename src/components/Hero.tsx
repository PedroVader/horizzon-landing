...
// Hook para el modal de Calendly
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white rounded-xl sm:rounded-2xl max-w-sm sm:max-w-2xl md:max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden shadow-2xl">
        {/* Header del modal - Más compacto en mobile */}
        <div className="flex items-center justify-between p-3 sm:p-6 border-b border-gray-200">
          <h3 className="text-lg sm:text-2xl font-bold text-[#222952]">
        ...
        <div className="p-2 sm:p-4 md:p-6">
          <div 
            ...
            style={{
              minWidth:'300px', 
              height: window.innerWidth < 640 ? '450px' : '600px'
            }}
          />
        </div>
      </div>
...
const Hero = () => {
  ...
  // Logos con traducciones
  const logos = [
    {
      src: "../images/PNG/remax.png",
      alt: t('hero.logos.alt1')
    },
    {
      src: "../images/PNG/neinor-homes.png",
      alt: t('hero.logos.alt2')
    },
    {
      src: "../images/PNG/new-logo-aedas.png",
      alt: t('hero.logos.alt3')
    },
    {
      src: "../images/PNG/VALENCIAREALESTATELOGOS-01.png",
      alt: t('hero.logos.alt4')
    },
    {
      src: "../images/PNG/Century-21-real-estate-Logo.png",
      alt: t('hero.logos.alt5')
    },
  ];

  return (
    <>
      <section className="h-screen max-h-[800px] min-h-[700px] md:min-h-screen relative flex items-center pt-20 md:pt-24 overflow-hidden">
        {/* Video de fondo */}
        <video
          autoPlay
          muted
        ...
        {/* Decoraciones - Más visibles en mobile */}
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 sm:px-6 lg:px-8">
          <div className="max-w-5xl">
            {/* Badge - Más grande en mobile */}
            <div className={`inline-flex items-center gap-3 bg-[#EBF0CB]/80 px-4 py-2.5 md:px-3 md:py-1.5 rounded-full mb-6 md:mb-6 border border-[#222952]/10 text-sm md:text-sm font-medium transition-all duration-1000 shadow-sm ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            }`}>
              <Award className="w-5 h-5 md:w-4 md:h-4 text-[#222952]" />
              <span className="text-[#222952] font-bold">
                {t('hero.badge')}
              </span>
            </div>

            {/* Título - MUCHO más grande en mobile */}
            <h1 className={`text-5xl sm:text-6xl md:text-6xl lg:text-7xl font-black tracking-tight leading-[1.1] mb-6 md:mb-6 transition-all duration-1000 ${
            ...
            {/* Subtítulo - Más grande y legible en mobile */}
            <p className={`text-lg sm:text-xl md:text-xl text-gray-700 font-medium max-w-2xl leading-relaxed mb-8 md:mb-8 transition-all duration-1000 delay-300 ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
            }`}>
              {t('hero.subtitle.main')}
              <span className="text-[#222952] font-bold mx-1 font-playfair italic">{t('hero.subtitle.highlight')}</span>
              {t('hero.subtitle.ending')}
            </p>

            {/* Carousel de logos - MUCHO más grande en mobile */}
            <div className={`mb-8 md:mb-10 transition-all duration-1000 delay-400 ${
                    ...
                    {logos.concat(logos).map((logo, index) => (
                      ...
                      >
                        <img
                          src={logo.src}
                          alt={logo.alt}
                          className="h-12 sm:h-14 md:h-12 w-auto object-contain grayscale hover:grayscale-0 transition-all duration-500 filter brightness-90"
                        />
                      </div>
                    ))}
                  </div>
            ...
            <div className={`flex flex-col sm:flex-row gap-4 md:gap-6 transition-all duration-1000 delay-500 ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
            }`}>
              {/* Botón principal - Abre modal de Calendly */}
              <button 
                onClick={openModal}
                className="group relative overflow-hidden bg-[#222952] text-white px-8 md:px-10 py-4 md:py-4 rounded-full font-bold text-lg md:text-base hover:scale-105 hover:shadow-xl transition-all duration-300 shadow-lg"
              >
        ...
        <style>{`
          ...
          /* Optimizaciones adicionales para mobile */
